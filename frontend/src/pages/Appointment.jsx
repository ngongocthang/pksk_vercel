...
//       toast.error("Bạn chưa có số điện thoại. Vui lòng cập nhật và quay lại sau!" || error.response?.data?.message);
//     }
//   };
...
const Appointment = () => {
  ...
  const handleBooking = () => {
    ...
    const patientId = user?.id || JSON.parse(localStorage.getItem("user"))?.id;
    const phone = user?.phone || JSON.parse(localStorage.getItem("user"))?.phone;
    ...
    if (!phone) {
      toast.info(
        <div className="flex flex-col items-center justify-center">
          ...
          <p>Bạn chưa cung cấp số điện thoại, vui lòng cập nhật.</p>
          <div className="flex justify-center gap-4">
            <button
              onClick={() => {
                navigate("/my-profile");
    ...
    toast.info(
      <div className="flex flex-col items-center justify-center">
        ...
        <p>
          Bạn có chắc chắn muốn đặt lịch hẹn vào {formattedDate} lúc {slotTime} không?
        </p>
        <div className="flex justify-center gap-4">
          <button
            onClick={() => {
